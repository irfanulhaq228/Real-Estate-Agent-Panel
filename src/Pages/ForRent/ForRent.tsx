...
import { homeForRentApi } from "../../Api/api";
...
const ForRent = () => {
  ...
  const fn_Submit = async () => {
    ...
    const result: any = await homeForRentApi(formData);
    if(result?.status === 200){
      navigate("/rental-homes");
  ...
  return (
    <div className="for-rent flex-1">
      <PagesHeader
        title="Request for Rental Home"
        nav={"Home For Rent"}
        subnav={"Create"}
      />
      <p className="m-5 font-[600] text-[grey] text-[13px]">
        Fill Form for Rent a Home
      </p>
      {/* section-1 */}
      <div className={"portion rounded m-5 p-5 flex flex-col gap-5"}>
        ...
        <div className="w-full flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(e) => fn_sectionOneData("monthlyPrice", e.target.value)}
          />
        </div>
        <div className="w-full flex flex-col gap-2">
          <label
